//------------------------------------------------------------------------------
// <auto-generated>
//     Este código se generó a partir de una plantilla.
//
//     Los cambios manuales en este archivo pueden causar un comportamiento inesperado de la aplicación.
//     Los cambios manuales en este archivo se sobrescribirán si se regenera el código.
// </auto-generated>
//------------------------------------------------------------------------------

namespace NavigationNetMedical.DBO
{
    using System;
    using System.Collections.Generic;
    
    public partial class vigilancia
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public vigilancia()
        {
            this.vigilanciaservice = new HashSet<vigilanciaservice>();
        }
    
        public string v_VigilanciaId { get; set; }
        public string v_PersonId { get; set; }
        public string v_PlanVigilanciaId { get; set; }
        public Nullable<int> i_WasNotifiedId { get; set; }
        public Nullable<int> i_ConfirmedNotification { get; set; }
        public string v_Commentary { get; set; }
        public Nullable<int> i_DoctorRespondibleId { get; set; }
        public Nullable<int> i_StateVigilanciaId { get; set; }
        public Nullable<System.DateTime> d_StartDate { get; set; }
        public Nullable<System.DateTime> d_EndDate { get; set; }
        public Nullable<int> i_IsDeleted { get; set; }
        public Nullable<int> i_InsertUserId { get; set; }
        public Nullable<System.DateTime> d_InsertDate { get; set; }
        public Nullable<int> i_UpdateUserId { get; set; }
        public Nullable<System.DateTime> d_UpdateDate { get; set; }
    
        public virtual person person { get; set; }
        public virtual planvigilancia planvigilancia { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<vigilanciaservice> vigilanciaservice { get; set; }
    }
}
